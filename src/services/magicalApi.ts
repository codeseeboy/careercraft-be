...
async function waitAndGetFinalResult(requestId: string): Promise<any> {
  console.log(`
...
Making single API call to get completed score...`);
  ...
  try {
    const finalResp = await axios.post(SCORE_URL, { request_id: requestId }, { headers: HEADERS });
    
    console.log(`Final Response:`, JSON.stringify(finalResp.data, null, 2));
    
    // Check if we have a final result (score data)
    if (finalResp.data?.data?.score !== undefined) {
      console.log(`
...
export async function callMagicalReview(url: string) {
  try {
    console.log(`
...
Headers:`, HEADERS);
    ...
    const resp = await axios.post(REVIEW_URL, requestBody, { headers: HEADERS });
...
export async function callMagicalScore(url: string, jobDescription: string) {
  try {
    console.log(`
...
Headers:`, HEADERS);
    ...
    const initialResp = await axios.post(SCORE_URL, requestBody, { headers: HEADERS });
...
Response Headers:`, initialResp.headers);
    ...
    const requestId = initialResp.data?.data?.request_id;
...
export async function callMagicalStatus(requestId: string) {
  try {
    console.log(`
...
Using same endpoint as score request per documentation`);
    ...
    const resp = await axios.post(SCORE_URL, requestBody, { headers: HEADERS });
