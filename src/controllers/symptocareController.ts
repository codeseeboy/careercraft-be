 ...
 */
export async function analyzeUserSymptoms(req: Request, res: Response) {
  ...
  try {
    ...
    console.log("✅ Received Gemini API response:", {
      extractedSymptoms: analysis.extractedSymptoms?.length || 0,
      possibleConditions: analysis.possibleConditions?.length || 0,
    ...
    const symptoCareResponse: SymptoCareResponse = {
      success: true,
      analysis: {
    ...
    try {
      ...
      const savedData = await datService.create(dbData);
    ...
    const errorResponse: SymptoCareResponse = {
      success: false,
      error: {
