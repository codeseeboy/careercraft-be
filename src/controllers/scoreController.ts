...
import { callMagicalScore } from "../services/magicalApi";

export async function scoreController(req: Request, res: Response) {
  try {
    ...
    if (req.body.url) {
      ...
      try {
        ...
        const data = await callMagicalScore(url, job_description);
